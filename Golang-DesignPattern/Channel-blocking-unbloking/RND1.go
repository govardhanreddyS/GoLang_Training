...
func downloadWorker(id int, tasks <-chan string, results chan<- Task, wg *sync.WaitGroup) {
	...
	for url := range tasks {
		...
		fmt.Printf("Worker %d downloaded content from %s and saved it to %s in %v\n", id, url, filePath, elapsed)
	}
}
...
func processWorker(id int, tasks <-chan Task, processedData *[]ProcessedData, wg *sync.WaitGroup) {
	...
	for task := range tasks {
		...
		*processedData = append(*processedData, ProcessedData{URL: task.URL, Processed: processed})
		...
		elapsed := time.Since(start)
		fmt.Printf("Worker %d processed data from %s in %v\n", id, task.URL, elapsed)
	}
}

func main() {
	startTotal := time.Now() // Start measuring total time

	var wg sync.WaitGroup

	// Define number of workers
	numDownloadWorkers := 10 // Increase the number of download workers
	numProcessWorkers := 60   // Increase the number of process workers
	...
	totalElapsed := time.Since(startTotal) // Calculate total time
