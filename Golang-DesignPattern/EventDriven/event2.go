...
func main() {
	// Create a channel with buffer to communicate events
	eventCh := make(chan Event, 10) // buffer size 10
...
// handleEvent dispatches the event to the appropriate handler based on its type
...
// subscribe adds a handler function to the event listener for all event types
func subscribe(ch <-chan Event, handler EventHandler) {
	go func() {
		for event := range ch {
			handler(event)
		}
...
// handleEventType1 is an example handler for EventType1 events
func handleEventType1(event Event) {
	// Simulate processing of EventType1 event
	time.Sleep(500 * time.Millisecond)
